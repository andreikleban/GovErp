...
| **GE-4** | Канал сценариев один: `GovErp.Application.Web` | Небольшой демонстрационный HTTP API в `GovErp.Web` — второй потребитель тех же сценариев и cookie-аутентификации. Пересмотреть, если API станет публичной интеграцией |
...
9. **Готовность к оплате.** Вычисляемый `ReadyForPaymentHandoff = Posted && PaidAt is null && VendorActive && !PaymentHold && DueDate <= BusinessDate`. BusinessDate передаётся явно (в демо 2026-06-15). Когда готовность выполнена, Pay записывает `PaidAt` и сбалансированный финансовый журнал по каждому фонду: дебет AP 2100, кредит Cash 1010, период — бизнес-дата; `SourceRef` — ссылка инвойса с суффиксом `/payment`. Статус остаётся Posted, статус Payable не хранить. Проверка остатка cash, банковские реквизиты и банковский файл не реализуются. Пересмотреть, когда модуль казначейства возьмёт выплату.
