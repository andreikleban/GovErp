...
public class RuleResolutionTests
{
    ...
    public void Resolves_latest_version_and_inclusive_date_and_scope()
    {
        ...
        var set = RuleResolution.Resolve([Rule(), latest, Rule("future", from: Date.AddDays(1)),
        ...
        RuleResolution.Resolve([latest], Date).Rules.Should().BeEmpty();
    }

    [Fact]
    public void Disabled_latest_version_does_not_resurrect_old_version() =>
        RuleResolution.Resolve([Rule(), Rule(version: 2, enabled: false)], Date).Rules.Should().BeEmpty();

    [Fact]
    public void Ambiguous_same_layer_version_is_rejected_even_with_different_scopes() =>
        FluentActions.Invoking(() => RuleResolution.Resolve([Rule(), Rule(fund: "101")], Date, "101"))
    ...
    public void Mandatory_guard_cannot_be_disabled_or_lowered(bool enabled, Severity severity) =>
        FluentActions.Invoking(() => RuleResolution.Resolve([Rule(), Rule(layer: RuleLayer.Tenant, enabled: enabled, severity: severity)], Date))
            .Should().Throw<ValidationException>();

    [Theory]
    [InlineData("threshold", "25000", "50000")]
    [InlineData("tolerance_pct", "0.05", "0.10")]
    public void Parameter_weakening_is_rejected_even_at_same_severity(string key, string baseline, string local) =>
        FluentActions.Invoking(() => RuleResolution.Resolve([
            Rule(parameters: new() { [key] = baseline }),
            Rule(layer: RuleLayer.Tenant, parameters: new() { [key] = local })], Date))
            .Should().Throw<ValidationException>();

    [Fact]
    public void Explicitly_overridable_rule_accepts_safe_stricter_parameters()
    {
        var local = Rule(layer: RuleLayer.Tenant, parameters: new() { ["threshold"] = "10000" });
        RuleResolution.Resolve([Rule(parameters: new() { ["threshold"] = "25000" }, roles: [ApproverRole.FinanceDirector]), local], Date)
    ...
    public void Fingerprint_is_order_independent_and_tracks_all_content()
    {
        ...
        var b = Rule("B");
        var first = RuleResolution.Resolve([a, b], Date);
        RuleResolution.Resolve([b, a], Date).Fingerprint.Should().Be(first.Fingerprint);
        RuleResolution.Resolve([Rule(parameters: new() { ["a"] = "1", ["b"] = "2" }), b], Date)
            .Fingerprint.Should().Be(first.Fingerprint);
        RuleResolution.Resolve([Rule(parameters: new() { ["a"] = "9", ["b"] = "2" }), b], Date)
            .Fingerprint.Should().NotBe(first.Fingerprint);
        RuleResolution.Resolve([Rule(message: "changed"), b], Date).Fingerprint.Should().NotBe(first.Fingerprint);
    ...
    public void Inputs_and_exposed_collections_are_immutable_snapshots()
    {
        ...
        var set = RuleResolution.Resolve(candidates, Date);
    ...
    {
        var first = RuleResolution.Resolve([Rule()], Date).Versions;
        var second = RuleResolution.Resolve([Rule()], Date).Versions;
    ...
    [InlineData("threshold", "25000", "50000")]
    [InlineData("tolerance_pct", "0.05", "0.10")]
    public void Explicit_override_still_cannot_weaken_parameter(string key, string baseline, string local) =>
        FluentActions.Invoking(() => RuleResolution.Resolve([
            Rule(parameters: new() { [key] = baseline }, roles: [ApproverRole.FinanceDirector]),
            Rule(layer: RuleLayer.Tenant, parameters: new() { [key] = local })], Date))
            .Should().Throw<ValidationException>();
    ...
    public void Fingerprint_changes_when_an_effective_field_changes(string field)
    {
        ...
        RuleResolution.Resolve([changed], Date, "101", "G").Fingerprint.Should()
            .NotBe(RuleResolution.Resolve([Rule()], Date, "101", "G").Fingerprint);
    }

    [Fact]
    public void Versions_include_every_selected_rule_not_only_layer_maxima()
    {
        var set = RuleResolution.Resolve([Rule("A", version: 2), Rule("B", version: 7),
    ...
    public void Override_role_order_does_not_change_fingerprint()
    {
        ...
        RuleResolution.Resolve([first], Date).Fingerprint.Should().Be(RuleResolution.Resolve([second], Date).Fingerprint);
    }
    [Fact]
    public void Mixed_scope_subject_retains_exact_selections_and_combined_fingerprint()
    {
        ...
        var global = Rule(parameters: new() { ["threshold"] = "25000" }, roles: [ApproverRole.FinanceDirector]);
        var scoped = Rule(layer: RuleLayer.Tenant, fund: "701", grant: "G-COPS-26",
            parameters: new() { ["threshold"] = "10000" });
        var union = RuleResolution.ResolveForSubject([global, scoped], subject);
        ...
        union.Find("X", "701", "G-COPS-26").Should().BeSameAs(scoped);
        FluentActions.Invoking(() => union.Find("X")).Should().Throw<ValidationException>();
        ...
        RuleResolution.ResolveForSubject([scoped, global], reversed).Fingerprint.Should().Be(union.Fingerprint);
        union.Fingerprint.Should().NotBe(RuleResolution.ResolveForSubject([global], subject).Fingerprint);
    ...
    public void Disjoint_scopes_may_use_same_rule_layer_and_version()
    {
        ...
        var set = RuleResolution.ResolveForSubject([first, second], subject);
        set.Rules.Should().Equal(first, second);
        RuleResolution.ResolveForSubject([second, first], subject with { Distributions = subject.Distributions.Reverse().ToArray() })
    ...
    public void Disabled_scoped_latest_version_does_not_reappear_from_union()
    {
        var subject = new SubjectBuilder()
            .With(SubjectBuilder.Distribution(1, "101-6000-53100", 10))
            .With(SubjectBuilder.Distribution(2, "701-6000-53100-G-COPS-26", 20)).Build();
        var set = RuleResolution.ResolveForSubject([Rule(), Rule(version: 2, fund: "701", enabled: false)], subject);
        ...
        set.Fingerprint.Should().NotBe(RuleResolution.ResolveForSubject([Rule()], subject).Fingerprint);
    }
    [Fact]
