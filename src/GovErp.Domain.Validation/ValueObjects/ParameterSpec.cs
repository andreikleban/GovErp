...
/// <summary>Which direction of a parameter makes its rule stricter.</summary>
...
{
    /// <summary>A positive amount, for example a threshold in USD.</summary>
    ...
    public string Requirement => Kind == ParameterKind.Amount ? "a positive amount" : "a share from 0 to 1";

    public bool Accepts(decimal value) => Kind == ParameterKind.Amount ? value > 0 : value is >= 0 and <= 1;

    /// <summary>A more local layer's value is valid and at least as strict as the value of the layer above it.</summary>
