...
public interface IBudgetAppService
{
    ...
    /// <summary>Line card: amounts, amendments, reservations (with invoice numbers) and encumbrances on this account. NotFound for an unknown account/year.</summary>
    Task<BudgetLineDetailVm> GetLineAsync(string account, int fiscalYear, ActorContext actor, CancellationToken ct = default);
    /// <summary>Amendment journal of all lines of the budget year (Budget › Amendments).</summary>
    Task<IReadOnlyList<AmendmentEntryVm>> ListAmendmentsAsync(int fiscalYear, ActorContext actor, CancellationToken ct = default);
    /// <summary>All tenant encumbrances (Budget › Encumbrances), regardless of the budget year.</summary>
